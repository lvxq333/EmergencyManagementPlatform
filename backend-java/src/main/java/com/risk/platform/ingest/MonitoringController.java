...
public class MonitoringController {
    ...
    public record StationInput(String code,String name,String region,Double longitude,Double latitude,String coordinateSystem) {}
    public record DeviceInput(String code,String name,Long stationId,String deviceType,String metricCode,Integer intervalSeconds) {}
    ...
    public Object addStation(@RequestHeader(value="Authorization",required=false) String auth,@RequestBody StationInput input) {authorize(auth,true,true);return service.saveStation(null,input);}
    @PutMapping("/stations/{id}") public Object editStation(@RequestHeader(value="Authorization",required=false) String auth,@PathVariable long id,@RequestBody StationInput input) {authorize(auth,true,true);return service.saveStation(id,input);}
    ...
    public Object addDevice(@RequestHeader(value="Authorization",required=false) String auth,@RequestBody DeviceInput input) {authorize(auth,true,true);return service.saveDevice(null,input);}
    @PutMapping("/devices/{id}") public Object editDevice(@RequestHeader(value="Authorization",required=false) String auth,@PathVariable long id,@RequestBody DeviceInput input) {authorize(auth,true,true);return service.saveDevice(id,input);}
    @PatchMapping("/devices/{id}/status") public Object status(@RequestHeader(value="Authorization",required=false) String auth,@PathVariable long id,@RequestBody StatusInput input) {authorize(auth,true,true);return service.status(id,input.enabled());}
    ...
    @DeleteMapping("/devices/{id}/simulation") public Object stopSimulation(@RequestHeader(value="Authorization",required=false) String auth,@PathVariable long id) {authorize(auth,true,true);return service.stopSimulation(id);}
